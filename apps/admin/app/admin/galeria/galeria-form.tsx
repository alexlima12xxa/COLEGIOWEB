      ...
      <div className="space-y-3">
        {items.map((item, idx) => (
          <CollapsibleCard
            key={idx}
            id={`galeria-item-${idx}`}
            title={item.title?.trim() || `Imagen ${idx + 1}`}
            defaultOpen={idx === 0}
            keepMounted
          >
            <fieldset className="space-y-3">
              <legend className="sr-only">Imagen {idx + 1}</legend>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <label htmlFor={`title-${idx}`} className="block text-sm font-medium text-zinc-700">
                    Título
                  </label>
                  <input
                    id={`title-${idx}`}
                  ...
                  />
                  <FieldError message={state.fieldErrors?.[`title-${idx}`]} />
                </div>
                <div>
                  <label htmlFor={`category-${idx}`} className="block text-sm font-medium text-zinc-700">
                    Categoría
                  </label>
                  <input
                    id={`category-${idx}`}
                    ...
                    className={inputClass}
                  />
                  <FieldError message={state.fieldErrors?.[`category-${idx}`]} />
                </div>
              </div>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <label htmlFor={`alt-${idx}`} className="block text-sm font-medium text-zinc-700">
                  ...
                  <input
                    ...
                    className={inputClass}
                  />
                  <FieldError message={state.fieldErrors?.[`alt-${idx}`]} />
                </div>
                <div>
                  <label htmlFor={`order-${idx}`} className="block text-sm font-medium text-zinc-700">
                    Orden
                  </label>
                  <input
                    id={`order-${idx}`}
