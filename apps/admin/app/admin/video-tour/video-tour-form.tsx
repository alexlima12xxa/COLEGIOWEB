...
import type { VideoTourState } from "./actions";
import { guardarVideoTour } from "./actions";
...
export function VideoTourForm({
  initial,
  posterJson,
}: {
  initial: VideoTourData;
  posterJson: string;
}) {
  const [state, formAction, pending] = useActionState<VideoTourState, FormData>(
    guardarVideoTour,
    {},
  );
  const [preview, setPreview] = useState<string | undefined>(
    mediaUrl(initial.poster),
  );
    ...
    <form action={formAction} className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">
          Imagen de portada del reproductor de video. Se sirve optimizada.
        </p>
      </div>
