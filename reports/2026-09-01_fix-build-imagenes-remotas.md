...
### Pasos
1. **Despublicar/eliminar las 3 filas de prueba** (`webhook-test`, `prueba-final`, `prueba-rebuild`) en el dashboard de Supabase (tabla `noticias`, tenant actual). *Manual, desbloquea el sitemap y el sitio público.*
2. **Verificar build limpio**: borrar `node_modules/.astro/assets` y ejecutar `pnpm build`. *Debe pasar: el archivo responde 200.*
